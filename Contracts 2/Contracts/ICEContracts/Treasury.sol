...
contract Treasury is Ownable {
    ...
    address public sAMZN;
    uint256 public sAMZNQueue; // Delays change to sAMZN address
    ...
    /**
          @notice allow approved address to deposit an asset for AMZN
          @param _amount uint
    ...
    function deposit(
        ...
        IERC20Mintable(AMZN).mint(msg.sender, send_);
    ...
    function withdraw(uint256 _amount, address _token) external {
        ...
        uint256 value = valueOfToken(_token, _amount);
        IAMZNERC20(AMZN).burnFrom(msg.sender, value);
    ...
    function incurDebt(uint256 _amount, address _token) external {
        ...
        uint256 maximumDebt = IERC20(sAMZN).balanceOf(msg.sender);
        // Can only borrow against sAMZN held
    ...
    /**
          ...
          @param _amount uint
       */
    function repayDebtWithAMZN(uint256 _amount) external {
        require(isDebtor[msg.sender], "Not approved");

        IAMZNERC20(AMZN).burnFrom(msg.sender, _amount);

        debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
        totalDebt = totalDebt.sub(_amount);

        emit RepayDebt(msg.sender, AMZN, _amount, _amount);
    }
    ...
    function mintRewards(address _recipient, uint256 _amount) external {
        ...
        IERC20Mintable(AMZN).mint(_recipient, _amount);
    ...
    function excessReserves() public view returns (uint256) {
        return totalReserves.sub(IERC20(AMZN).totalSupply().sub(totalDebt));
    }
    ...
    returns (uint256 value_)
    {
        if (isReserveToken[_token]) {
            ...
            value_ = _amount.mul(10 ** IERC20(AMZN).decimals()).div(
    ...
    returns (bool)
    {
        ...
        } else if (_managing == MANAGING.SAMZN) {
            // 9
            sAMZNQueue = block.number.add(blocksNeededForQueue);
        } else return false;
