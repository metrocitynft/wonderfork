...
contract AmznBondDepository is Ownable {
    ...
    IERC20 public immutable Amzn; // token given as payment for bond
    ...
    function deposit( 
        ...
        uint balanceBefore = Amzn.balanceOf(address(this));
        /**
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            Amzn.safeTransfer( DAO, fee ); 
        }
        require(balanceBefore.add(profit) == Amzn.balanceOf(address(this)), "Not enough Amzn to cover profit");
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            Amzn.transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                Amzn.approve( address(stakingHelper), _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return Amzn.totalSupply().mul( terms.maxPayout ) / 100000 ;
    }
