...
function Wrap({ open, handleClose }: IAdvancedSettingsProps) {
    ...
    const samznAllowance = useSelector<IReduxState, number>(state => {
        return state.account.wrapping && state.account.wrapping.samzn;
    });
    ...
    const hasAllowance = useCallback(() => samznAllowance > 0, [samznAllowance]);

    const trimmedSamznBalance = trim(Number(samznBalance), 6);
    const trimmedWmemoBalance = trim(Number(wmemoBalance), 6);

    const getBalance = () => (isWrap ? `${trimmedSamznBalance} MEMO` : `${trimmedWmemoBalance} wMEMO`);
            ...
            <Paper className="ohm-card ohm-popover wrap-token-poper">
                <div className="cross-wrap wrap-cros-wrap">
                    ...
                    <div className="wrap-price" onClick={() => setIsWrapPrice(!isWrapPrice)}>
                        <p>
                            1 {isWrapPrice ? "SAMZN" : "wMEMO"} = {`${trim(wrapPrice, 4)} ${isWrapPrice ? "wMEMO" : "SAMZN"}`}
                        </p>
                    </div>
                </div>
