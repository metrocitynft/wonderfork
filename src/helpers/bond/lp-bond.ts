...
export class LPBond extends Bond {
    ...
    public getAmznAmount(networkID: Networks, provider: StaticJsonRpcProvider) {
    ...
    private async getReserves(networkID: Networks, provider: StaticJsonRpcProvider, isToken: boolean): Promise<number> {
        ...
        const isAmzn = token1.toLowerCase() === addresses.AMZN_ADDRESS.toLowerCase();

        return isToken ? this.toTokenDecimal(false, isAmzn ? reserve0 : reserve1) : this.toTokenDecimal(true, isAmzn ? reserve1 : reserve0);
    }

    private toTokenDecimal(isAmzn: boolean, reserve: number) {
        return isAmzn ? reserve / Math.pow(10, 9) : reserve / Math.pow(10, 18);
    }
}
