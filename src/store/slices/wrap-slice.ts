...
export const changeApproval = createAsyncThunk("wrapping/changeApproval", async ({ provider, address, networkID }: IChangeApproval, { dispatch }) => {
    ...
    const samznContract = new ethers.Contract(addresses.SAMZN_ADDRESS, wMemoTokenContract, signer);

    let approveTx;
    try {
        const gasPrice = await getGasPrice(provider);

        approveTx = await samznContract.approve(addresses.WMEMO_ADDRESS, ethers.constants.MaxUint256, { gasPrice });
    ...
    const wmemoAllowance = await samznContract.allowance(address, addresses.WMEMO_ADDRESS);
...
const calcWrapValue = async ({ isWrap, value, provider, networkID }: IWrapDetails): Promise<number> => {
    ...
    if (isWrap) {
        ...
        const samznValue = await wmemoContract.wMEMOToMEMO(amountInWei);
        wrapValue = samznValue / Math.pow(10, 9);
    }
